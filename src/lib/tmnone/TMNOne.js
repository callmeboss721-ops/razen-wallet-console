...
class TMNOne {
	...
	*/
	...
	amount = จำนวนเงิน
	*/
    async transferQRPromptpay(payee_proxy_value, amount, personal_msg = '') {
        try {
            const amount_str = parseFloat(amount).toFixed(2);
            
            let uri = 'transfer-composite/v2/promptpay/draft';
            let signature = await this.calculate_sign256(`/tmn-mobile-gateway/${uri}|${this.#wallet_access_token}|${amount_str}|${payee_proxy_value}|${personal_msg}`);
            ...
            const draft_transaction_id = wallet_response_body.data.payment_reference_id;

            uri = 'transfer-composite/v2/promptpay/transfer';
            ...
            post_body = JSON.stringify({
                payment_reference_id: draft_transaction_id
            });
    ...
    async #tmnone_connect(request_body_json) {
        ...
        try {
            ...
            const headers = {
                ...
                'Content-Type': 'application/json',
                'User-Agent': `okhttp/4.4.0/202607032100/${this.#tmnone_keyid}`
            };
